"""
steering.py
Numpy-based implementations of common steering behaviors: seek, arrive, pursuit, evade, separation, cohesion, obstacle avoidance.
These functions are small and allocation-light; they return desired velocity vectors.
"""
import numpy as np


def seek(agent_pos: np.ndarray, target_pos: np.ndarray, max_speed: float) -> np.ndarray:
...
def arrive(agent_pos: np.ndarray, target_pos: np.ndarray, max_speed: float, slowing_radius: float) -> np.ndarray:
...
def pursuit(agent, target_agent, max_speed: float) -> np.ndarray:
...
def evade(agent, pursuer, max_speed: float) -> np.ndarray:
...
def separation(agent, neighbors, desired_separation: float, max_force: float) -> np.ndarray:
...
def cohesion(agent, neighbors, max_speed: float) -> np.ndarray:
...
def obstacle_avoidance(agent, obstacles, avoid_distance: float, max_force: float) -> np.ndarray:
    steer = np.zeros(2, dtype=np.float64)
