...
def main_menu(screen, clock, width, height):
    ...
    buttons = [
        (pygame.Rect(width // 2 - 180, int(height * 0.38), 360, 68), "Einzelspieler", "singleplayer"),
        (pygame.Rect(width // 2 - 180, int(height * 0.50), 360, 68), "LAN Multiplayer", "multiplayer"),
        (pygame.Rect(width // 2 - 180, int(height * 0.62), 360, 68), "Beenden", "quit"),
    ]
    while True:
        ...
        hint = pygame.font.Font(None, 22).render("1 = Einzelspieler   2 = LAN Multiplayer   ESC = Beenden", True, (150, 155, 170))
...
def lobby_screen(screen, clock, session, width, height):
    """Show the shared lobby. Returns 'start', 'back' or 'host_disconnected'."""
    ...
    while True:
        ...
        can_start = session.is_host() and session.connected and session.phase == "lobby"
        mouse = pygame.mouse.get_pos()
        ...
        screen.fill((7, 8, 20))
        title = title_font.render("STAR WARS – LAN LOBBY", True, (255, 255, 255))
        ...
        y = panel.y + 28
        header = font.render(f"Verbunden: {len(players)} / 10", True, (160, 200, 230))
        ...
        if not session.is_host():
            hint = small_font.render("Der Host startet das Spiel.", True, (160, 165, 180))
