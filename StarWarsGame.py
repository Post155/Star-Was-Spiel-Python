...
while True:
    ...
    while running:
        ...
        # Draw enemies (managed by EnemyManager)
        enemy_manager.draw_all(screen)

        # Update enemy AI before rendering explosions / score
        player_state = {
            'position': np.array([spieler.x + spieler.width / 2, spieler.y + spieler.height / 2], dtype=np.float32),
            'velocity': np.array([0.0, 0.0], dtype=np.float32),
            'heading': 0.0,
        }
        other_world = {
            'nearest_ally_distance': 900.0,
        ...
        # Convert projectile dicts into in-game EnemyLaser objects
        for p in enemy_projectiles:
            if p.get('type') == 'laser':
                x, y = p.get('pos', (0, 0))
                vx, vy = p.get('vel', (0.0, 6.0))
                # EnemyLaser class available via game.entities
                enemy_laser = EnemyLaser(x, y, vx=vx, vy=vy)
